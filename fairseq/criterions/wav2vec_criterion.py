...
class Wav2VecCriterionConfig(FairseqDataclass):
    ...
    code_book_balance: str = field(
        default='None',
        metadata={
            "help": "['None', 'focal', 'class_reweight']"
        },
    )

@register_criterion("wav2vec", dataclass=Wav2VecCriterionConfig)
class Wav2vecCriterion(FairseqCriterion):
    def __init__(self, task, infonce=False, loss_weights=None, log_keys=None, code_book_balance='None'):
        super().__init__(task)
        self.infonce = infonce
        self.loss_weights = loss_weights
        self.log_keys = [] if log_keys is None else log_keys
        self.code_book_balance = code_book_balance
    ...
    def forward(self, model, sample, reduce=True):
        ...
        logits = model.get_logits(net_output).float() # [14352, 101]
        target = model.get_targets(sample, net_output) # [14352]
        target_features = model.get_target_features(net_output) # [48, 299, 256]
        hard_x = model.get_targets_for_weighting(net_output) # B,2,320 # [14352, 2, 320]
        avg_probs = model.get_avg_probs(net_output) # B,2,320 # [14352, 2, 320]
        ...
        if self.code_book_balance=="class_reweight":
            ...
            if torch.randperm(1000)[0] == 0:
                print('hard_x distribution:', hard_x_dist) # 2,320
                print('avg_probs distribution:', avg_probs) # 2,320
        ...
        if self.infonce:
            loss = F.cross_entropy(logits, target, reduce=False)
            loss = (loss*sample_weight.detach()).sum()
        else:
