...
class Wav2Vec2Model(BaseFairseqModel):
    def __init__(self, cfg: Wav2Vec2Config):
        ...
        # self.class_weight = torch.nn.Parameter(torch.zeros(cfg.latent_groups * cfg.latent_vars), requires_grad=True)
    ...
    def forward(
        ...
        if self.quantizer:
            if self.negatives_from_everywhere:
                ...
                if "hard_x" in q:
                    hard_x = q["hard_x"]
                if "prob_perplexity_loss" in q:
                    loss_calibration = q["prob_perplexity_loss"]
                if "zero_mean_loss" in q:
                    loss_zero_mean = q["zero_mean_loss"]
        ...
        # x: torch.Size([40, 342, 256])
        # y: torch.Size([40, 342, 256])
        # negs: torch.Size([100, 40, 342, 256])
        ...
        if hard_x is not None:
            result["hard_x"] = hard_x

        if loss_zero_mean is not None:
            result["loss_zero_mean"] = loss_zero_mean
    ...
    def get_logits(self, net_output):
        logits = net_output["x"]
        # raise ValueError('x size:', logits.size()) # [101, 48, 305]
    ...
    def get_targets_for_weighting(self, net_output):
        return net_output["hard_x"]

    def get_avg_probs(self, net_output):
        return net_output["avg_probs"]
    
    def get_features(self, net_output):
        return net_output["features"]
    
    def get_target_features(self, net_output):
        return net_output["y"]
    ...
    def get_extra_losses(self, net_output):
        ...
        if "loss_zero_mean" in net_output:
            pen.append(net_output["loss_zero_mean"])
